...
- `skills/full-development/references/code-clarity-guide.md`：仅在命名、文件边界或注释需要系统治理时加载的触达范围指南。
...
8. 代码类修改在触达范围内检查命名、文件职责和注释；只有系统性问题才加载详细指南或升级范围。
9. 结束时用已有证据执行最小收尾；代码类修改生成 Commit 文稿，但不自动提交或推送。
10. 将执行时间、等级、状态和分级结果写入当天 worklog；L0/L1 由根规则直接记录，L2-L5 按需加载 `record-task-log`。
11. 复杂 L2/演进型 L3 按需、L4/L5 强制使用 `track-change-trace`，在当天 TRACE 追加开始、实质调整和最终映射。
...
- 每天每个项目只有一个 worklog 文件；记录从旧到新排列且不拥有当前项目事实。
- 每天每个项目只有一个 TRACE 文件；检查点从旧到新排列且不可改写，结论变化通过更正、取代或末尾的继续事件表达。
