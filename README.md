...
代码清晰度治理只检查本轮新增或实质修改的范围：优先让命名表达领域意图，让文件边界对应职责、依赖与变化原因，并只用注释补充契约、原因和不变量。普通任务不做全仓命名巡检，也不按固定行数机械拆文件；需要系统治理时再按需加载详细指南。

当天的 worklog 与 TRACE 均按真实时间从旧到新排列，新的内容放在文件下方。补录较早任务时按真实时间归位；重新处理旧任务时在文件末尾追加同一任务编号的“继续”记录。审计脚本会识别时间倒序和不再支持的 L0/L1 列表格式。
...
- 代码清晰度检查限制在触达范围，按职责和变化原因判断边界，不制造全仓整理任务。
- 每日 worklog 和 TRACE 从上到下按时间递增，新增记录通常位于文件末尾。
