...
本包用根目录 `AGENTS.md` 作为始终加载的轻量控制层，用五个按需 Agent Skill 分别处理项目初始化、精准修复、完整开发、知识同步和分级任务日志。适用于支持 `AGENTS.md` 与开放 `SKILL.md` 格式的 Agent；不支持原生 Skill 加载时，可按 `AGENTS.md` 指引直接读取对应 Skill。
