...
本包用根目录 `AGENTS.md` 作为始终加载的轻量控制层，用七个按需 Agent Skill 分别处理项目初始化、精准修复、完整开发、按需规格驱动、知识同步、分级任务日志和修改轨迹。适用于支持 `AGENTS.md` 与开放 `SKILL.md` 格式的 Agent；不支持原生 Skill 加载时，可按 `AGENTS.md` 指引直接读取对应 Skill。
...
需要长期行为契约的 L3-L5 变更可使用 `spec-driven-change`：以轻量或完整 Spec、稳定规范 ID 和就近追溯表连接验收、测试、实现与验证。普通局部缺陷和行为不变重构默认不创建 Spec，也不通读历史规格。
...
- Spec 只在持久行为契约需要时创建，并按功能拆分，避免全仓巨型追溯矩阵。
